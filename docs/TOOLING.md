...
| `transcription/reauth_youtube.py` | Mints a fresh `YT_REFRESH_TOKEN` (loopback OAuth, `youtube.force-ssl`) and rewrites it into `tsd-secrets.env`, verifying the refresh round-trips. Needed whenever the token returns `invalid_grant` — Google expires refresh tokens after 7 days while the consent screen is still in *Testing*; publishing the app stops that recurring. |
