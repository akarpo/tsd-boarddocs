...
| `transcription/upload_videos.py` | Full video upload (`videos.insert` resumable, 1,600 units; prints the id before the thumbnail step, which is skipped with a note when the stream has no crest card and no `--date`) + crest-card thumbnail via `thumbnails.py`; same OAuth. Used for TelVue-only meetings the channel lacks. Pass `--date`/`--name` so a card can be typeset when the stream shows none. |
| `transcription/forum_doc.py` | The League of Women Voters candidate-forum deliverable: a spec (title block, questions with timestamps, notes) + the repaired attributed-utterance JSON → `.docx` (python-docx, the 2024 layout) and `.pdf` (headless Chrome; Word's export times out). Editions under `transcription/examples/lwv-<date>/`. |
